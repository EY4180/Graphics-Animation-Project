...
/**
 * Sets currButton shared variable to entire program to identify the current
 * state of the mouse.
 * 
 * Notes: accessed by mouse motion function to determine mouse movement during
 * a mouse action.
 */
...
static void adjustcamSideUp(vec2 su)
{
    ...
    camRotUpAndOverDeg = fmin(camRotUpAndOverDeg, 85.0);
    camRotUpAndOverDeg = fmax(camRotUpAndOverDeg, -85.0);
}
...
static void adjustScaleY(vec2 sy)
{
    ...
    printf("y: %f scale: %f\n", sceneObjs[toolObj].loc[1], sceneObjs[toolObj].scale);
}
...
void display(void)
{
    ...
    for (int i = 0; i < nObjects; i++)
    {
        ...
        glUniform3fv(glGetUniformLocation(shaderProgram, "GlobalAmbient"), 1, globalAmbient);
        CheckError();
...
void reshape(int width, int height)
{
    ...
    GLfloat fovVariable = atan(aspectRatio / nearDist) / DegreesToRadians;

    mat4 perspProjection = Perspective(fovVariable, aspectRatio, nearDist, 100.0);
    // swap appropriate variables
    if (width < height)
    {
        GLfloat temp = perspProjection[0][0];
        perspProjection[0][0] = perspProjection[1][1];
        perspProjection[1][1] = temp;
    }
